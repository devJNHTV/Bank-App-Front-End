...
export class CurrentRepaymentScheduleComponent implements OnInit {
  ...
  calculateTotalAmount(): number {
    if (!this.currentRepayment) return 0;
    return this.currentRepayment.principal + this.currentRepayment.interest;
  }

  calculateRemainingAmount(): number {
    if (!this.currentRepayment) return 0;
    return this.calculateTotalAmount() - this.currentRepayment.paidAmount;
  }
